...
#define at3(rx, ry, rz) ( rx * q[0] + ry * q[1] + rz * q[2] )
	...
	GLubyte* CNoise::Noise3DTexPtr = 0;	
	int CNoise::p[MAXB + MAXB + 2] = { 0 };
	double CNoise::g3[MAXB + MAXB + 2][3] = { 0 };
	double CNoise::g2[MAXB + MAXB + 2][2] = { 0 };
	double CNoise::g1[MAXB + MAXB + 2] = { 0 };
	int CNoise::start = 0;
	int CNoise::B = 0;
	int CNoise::BM = 0;

	void CNoise::CreateNoise3D()
	{
		make3DNoiseTexture();
		init3DNoiseTexture(Noise3DTexSize, Noise3DTexPtr);
	}

	void CNoise::SetNoiseFrequency(int frequency)
	{
		start = 1;
		B = frequency;
		BM = B - 1;
	...
	double CNoise::PerlinNoise1D(double x, double alpha, double beta, int n)
	{
		int i;
		double val, sum = 0;
		double p, scale = 1;
	...
	double CNoise::PerlinNoise2D(double x, double y, double alpha, double beta, int n)
	{
		int i;
		double val, sum = 0;
		double p[2], scale = 1;
	...
	double CNoise::PerlinNoise3D(double x, double y, double z, double alpha, double beta, int n)
	{
		int i;
		double val, sum = 0;
		double p[3], scale = 1;
	...
	void CNoise::make3DNoiseTexture()
	{
		int f, i, j, k, inc;
	...
	void CNoise::init3DNoiseTexture(int texSize, GLubyte* texPtr)
	{
